                ...
                <div class="container">
                    ...
                    <form action="{{ route('franchise.invoice.store') }}" method="POST" id="invoice-form">
                        ...
                        <div class="row mb-4">
                            ...
                            <div class="col-md-3">
<button type="button" id="add-item" class="btn btn-primary action-btn">Add Item</button>

                            </div>
                        </div>
                        <button class="btn btn-success action-btn">Create Invoice</button>
                    </form>
                </div>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const taxRate = parseFloat('{{ $franchisee }}'); // Use the correct tax rate

    // Function to update the totals
    function updateTotals() {
        let subtotal = 0;

        // Calculate the subtotal by summing the item totals
        ...
        // Calculate tax and total if checkbox is checked
        if (applyTaxCheckbox.checked) {
            const tax = subtotal * (taxRate / 100); // Calculate sales tax
            taxAmountEl.textContent = tax.toFixed(2);
            taxRow.style.display = ''; // Show the sales tax row
            totalEl.textContent = (subtotal + tax).toFixed(2); // Update total
        } else {
            taxRow.style.display = 'none'; // Hide the sales tax row if tax is not applied
            totalEl.textContent = subtotal.toFixed(2); // Just show the subtotal as total
        }
    }
...
`;
        ...
        updateTotals(); // Update totals whenever an item is added
    });
    ...
    itemsTableBody.addEventListener('click', e => {
        if (e.target.classList.contains('remove-item')) {
            ...
            updateTotals(); // Update totals whenever an item is removed
        }
    });

    // Event listener for the sales tax checkbox
    applyTaxCheckbox.addEventListener('change', updateTotals);

    // Initial call to update totals when the page loads
    updateTotals(); // This will be triggered automatically on page load
});
