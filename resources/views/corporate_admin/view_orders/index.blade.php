                                ...
                                <tbody>
                                    @foreach ($orders as $order)
                                        <tr>
                                            ...
                                            <td>
                                                <div class="d-flex">
                                                    <form action="{{ route('corporate_admin.vieworders.destroy', $order->date_transaction) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this order?')">
                                                        @csrf
                                                        @method('DELETE')
...
document.addEventListener("DOMContentLoaded", function() {
    document.querySelectorAll('.status-select').forEach(select => {
        select.addEventListener('change', function() {
            let dateTransaction = this.getAttribute('data-date');  // Get the date_transaction value
            ...
            fetch(`/corporate_admin/vieworders/update-status`, { 
                ...
                body: JSON.stringify({ date_transaction: dateTransaction, status: newStatus })
            })
            .then(response => response.json())
            .then(data => alert(data.message))  // Show a message when the update is successful
